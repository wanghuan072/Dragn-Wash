...
const keyResponse = await fetchWithRetry(new URL(indexNowKeyPath, currentOrigin));
if (!keyResponse.ok || (await keyResponse.text()).trim() !== indexNowConfig.key) {
  throw new Error(
...
if (!shouldSubmit) {
  console.log(`Dry run complete; ${canonicalUrls.length} URL(s) would be submitted.`);
  process.exit(0);
}
...
async function compareDeployments(previous, current) {
  ...
  const sharedPaths = currentPaths.filter((pathname) => previousSet.has(pathname));
...
async function readSitemap(origin) {
  ...
  if (!response.ok) {
    throw new Error(`Could not read ${origin}/sitemap.xml (${response.status}).`);
  }
...
async function readPage(origin, pathname) {
  ...
  if (!response.ok) {
    throw new Error(`Expected 200 for ${origin}${pathname}, received ${response.status}.`);
  }
...
function fingerprintHtml(html) {
  ...
  const canonical =
    [...html.matchAll(/<link\b[^>]*>/gi)]
      ...
      .find((attributes) => attributes.rel?.toLowerCase() === "canonical")?.href ?? "";
...
async function waitForCanonicalDeployment(changes, sourceOrigin, publicOrigin) {
  ...
  for (const item of changes) {
    if (item.change !== "deleted") {
      expected.set(item.pathname, (await readPage(sourceOrigin, item.pathname)).fingerprint);
    }
  }
...
function normalizeOrigin(value) {
  if (typeof value !== "string") throw new Error("A deployment origin is required.");
